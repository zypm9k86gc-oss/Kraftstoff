...
assert.equal($('#roadMotorway').checked,false);
assert.equal($('#roadCountry').checked,false);
assert.equal($('#roadCity').checked,false);
$('#roadMotorway').checked=true; $('#roadCity').checked=true;
...
assert.deepEqual(Array.from(changed.roadTypes),['Motorway','City']);
...
context.editTrip('old');
assert.equal($('#roadMotorway').checked,true);assert.equal($('#roadCity').checked,true);assert.equal($('#roadCountry').checked,false);
$('#roadMotorway').checked=false;$('#roadCity').checked=false;
assert.deepEqual(Array.from(context.tripFromForm().roadTypes),[]);
const html=fs.readFileSync(__dirname+'/index.html','utf8');
const metricBlock=html.slice(html.indexOf('class="metric-fields"'),html.indexOf('id="accEnabled"'));
assert.deepEqual([...metricBlock.matchAll(/<input id="([^"]+)"/g)].map(m=>m[1]),['consumption','duration','averageSpeed','distance']);
